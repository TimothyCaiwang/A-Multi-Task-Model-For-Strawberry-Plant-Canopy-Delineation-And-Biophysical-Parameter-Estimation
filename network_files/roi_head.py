...
def biomass_loss(bio_logits, gt_biomasses, pos_matched_idxs):
    # type: (Tensor, List[Tensor], List[Tensor]) -> Tensor
    gt_biomass = [gt_bio[idxs] for gt_bio, idxs in zip(gt_biomasses, pos_matched_idxs)]
    gt_biomass = torch.cat(gt_biomass)
    gt_biomass = torch.where(torch.isnan(gt_biomass), torch.full_like(gt_biomass, 0), gt_biomass)
    gt_biomass = torch.unsqueeze(gt_biomass, 1)
    criterion1 = torch.nn.L1Loss() # L1Loss() #  nn.MSELoss()
    bio_loss = 0.5*criterion1(bio_logits, gt_biomass)
    return bio_loss


def proposals_to_boxes(proposals):
    widths = proposals[:, 2] - proposals[:, 0]  # anchor/proposal宽度
    heights = proposals[:, 3] - proposals[:, 1]  # anchor/proposal高度
    ctr_x = proposals[:, 0] + 0.5 * widths  # anchor/proposal中心x坐标
    ctr_y = proposals[:, 1] + 0.5 * heights  # anchor/proposal中心y坐标
    x1 = ctr_x - torch.tensor(42, dtype=ctr_x.dtype, device=ctr_x.device)
    y1 = ctr_y - torch.tensor(41, dtype=ctr_x.dtype, device=ctr_x.device)
    x2 = ctr_x + torch.tensor(42, dtype=ctr_x.dtype, device=ctr_x.device)
    y2 = ctr_y + torch.tensor(41, dtype=ctr_x.dtype, device=ctr_x.device)
    # pre_boxes = [x1, y1, x2, y2]

    pre_boxes = torch.stack((x1, y1, x2, y2), dim=1)
    pre_boxes_c = torch.clamp(pre_boxes, 0.0, 800.0)
    return pre_boxes_c
...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
        ...
        super(RoIHeads, self).__init__()  # 继承了nn.module父类的初始化方法
        ...
        self.bio_roi_pool = bio_roi_pool
        self.bio_head = bio_head
        self.bio_predictor = bio_predictor
    ...
    def has_biomass(self):
        if self.bio_roi_pool is None:
            return False
        if self.bio_head is None:
            return False
        if self.bio_predictor is None:
            return False
        return True
    ...
    def forward(self,
        ...
        if self.has_biomass():
            biomass_proposals = [p["boxes"] for p in result]  # 将最终预测的Boxes信息取出
            if self.training:
                # matched_idxs为每个proposal在正负样本匹配过程中得到的gt索引(背景的gt索引也默认设置成了0)
                if matched_idxs is None:
                    raise ValueError("if in training, matched_idxs should not be None")
                ...
                bio_matched_idxs = []
                for img_id in range(num_images):
                    bio_pos = torch.where(labels[img_id] > 0)[0]  # 寻找对应gt类别大于0，即正样本
                    proposals_o = proposals[img_id][bio_pos]
                    proposals_m = proposals_to_boxes(proposals[img_id][bio_pos])
                    # proposals_to_boxes(proposals[img_id][bio_pos])
                    biomass_proposals.append(proposals_m)
                    bio_matched_idxs.append(matched_idxs[img_id][bio_pos])
            else:
                bio_matched_idxs = None

            # biomass_boxes = proposals_to_boxes(biomass_proposals)
            bio_features = self.bio_roi_pool(features, biomass_proposals, image_shapes)
            bio_features = self.bio_head(bio_features)
            bio_logits = self.bio_predictor(bio_features)
            t = 1

            loss_biomass = {}
            if self.training:
                if targets is None or pos_matched_idxs is None or bio_logits is None:
                    raise ValueError("targets, pos_matched_idxs, mask_logits cannot be None when training")

                gt_biomass = [t["biomass"] for t in targets]
                t = 1
                rcnn_loss_biomass = biomass_loss(bio_logits, gt_biomass, pos_matched_idxs)
                loss_biomass = {"loss_biomass": rcnn_loss_biomass}
            else:
                t = 0
                objects_per_image = [objects_in_image.shape[0] for objects_in_image in biomass_proposals]
                bio_logits_list = bio_logits.split(objects_per_image, 0)
                num_images = len(biomass_proposals)
                for i in range(num_images):
                    for biomass_i, r in zip(bio_logits_list, result):
                        r["biomass"] = biomass_i
                        it = 0
                    ...
                    result.append(
                        {
                            "biomass": bio_logits_list[i]
                        }
                    )
                    '''
            losses.update(loss_biomass)
